...
pub struct Feeder {
    pub sf: Statfeed<Rc<Sound>>,
    pub phonemes_per_line: usize,
    current_line: usize
}

impl Feeder {
    pub fn new(sounds: Vec<Rc<Sound>>, phonemes_per_line: usize, lines: usize) -> Feeder {
        let n = phonemes_per_line * lines;
        let max_power = sounds.iter().map(|s| s.max_power())
            .fold(f64::NAN, |acc, x| x.max(acc)); 
        ...
        let weights = (0..lines).fold(Vec::<Vec<f64>>::new(), |mut weights, line| {
            ...
            let power_weight = (phase * 2. + f64::consts::PI).cos(); // in [-1, 1]
            let pitch_weight = (phase + f64::consts::PI).cos(); // in [-1, 1]
            let w: Vec<f64> = sounds.iter().map(|s| {
                let pitch_confidence = s.pitch_confidence();
                let power = s.max_power();
                let normed_power = (s.max_power() / max_power) * 2. - 1.; // in [-1, 1]
                let normed_pitch = (s.pitch_confidence() / max_con) * 2. - 1.; // in [-1, 1]
                (((normed_power * power_weight) + (normed_pitch * pitch_weight)) + 2.) * 0.25
            }).collect();
        ...
        let mut feeder = Feeder {
            sf: Statfeed::<Rc<Sound>>::new(sounds, n),
